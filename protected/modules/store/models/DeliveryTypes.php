 ...
 *
 * The followings are the available columns in table 'delivery_types':
 * @property integer $id
 * @property string $alias
 * @property string $name
 * @property string $is_used
 */
class DeliveryTypes extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('alias, is_used', 'length', 'max'=>255),
			array('name', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, alias, name, is_used', 'safe', 'on'=>'search'),
		);
	}
