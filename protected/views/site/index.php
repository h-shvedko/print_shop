...
<? endif;?>
	<? for($i = (int)FALSE; $i < $j; $i++): ?>
		...
		<? if ($i == 0 || $i == 3 || $i == 6 || $i == 9 || $i == 12 || $i == 15 || $i == 18) : ?>
			<div class="row-fluid">
		<? endif;?>
			<? if ($catalog[$i] instanceof Catalog) : ?>
				<div class="span4 blocks" onclick="location.href='<?php echo Yii::app()->request->baseUrl; ?>/page/default/index/id/<?=$catalog[$i]->id?>';">
					<div class="row-fluid">
					...
					<div class="row-fluid">
						...
						<div class="span6">
							<div class="price">
								<p>Тираж: <i class="count"><?=CHtml::encode($catalog[$i]->tirazh1);?></i> шт.</p>
								<p style="margin-bottom: 30px">Цена: <i class="amount"><?=CHtml::encode($catalog[$i]->price1);?></i> грн.</p>
								
								<p>Тираж: <i class="count"><?=CHtml::encode($catalog[$i]->tirazh2);?></i> шт.</p>
								<p>Цена:  <i class="amount"><?=CHtml::encode($catalog[$i]->price2);?></i> грн.</p>
							</div>
						</div>
					</div>
				</div>
